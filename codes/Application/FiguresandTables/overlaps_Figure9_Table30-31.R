...
for (d in start:end){
  ...
  pdf(paste(DIRECOUT, "FiguresandTables/overlaps_window", d, ".pdf", sep=""))  ###where overlap plots are stored
...
write.csv(weightSum, paste(DIRECOUT, "FiguresandTables/weight_sample.csv", sep=""))  ###summary of weights across all the windows
write.csv(overlapProp, paste(DIRECOUT, "FiguresandTables/overlapProportion.csv", sep="")) ##summary of overlap proportions between treatment groups
...
weightSum=read.csv(paste(DIRECOUT, "FiguresandTables/weight_sample.csv", sep=""), header=T)
weightSum=format(weightSum, digits=3)

outPut=weightSum[7:21,]
...
write.table(outPut, paste(DIRECOUT, "FiguresandTables/weight.txt",sep=""),  quote=F, col.names=F, row.names=F)



############################to create Table 31 in the appendix
propSum=read.csv(paste(DIRECOUT, "FiguresandTables/overlapProportion.csv", sep=""), header=T)
propSum=propSum*100
...
write.table(outPut, paste(DIRECOUT, "FiguresandTables/overlapProportion.txt",sep=""),  quote=F, col.names=F, row.names=F)
