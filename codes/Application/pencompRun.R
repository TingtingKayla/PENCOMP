...
for (d in start:end) {
  ...
  sampleNumBoot=matrix(NA, nrow=numLoop,ncol=8)  #### number of samples in each of the four treatment combinations
    ...
    tryCatch ( 
      {
        ...
        sampleNumBoot[h,]=c(sum(simdatFit$A1==1 & simdatFit$A2==1),
                            sum(simdatFit$A1==1 & simdatFit$A2==0),
                            sum(simdatFit$A1==0 & simdatFit$A2==1),
  ...
  sampleNum[d,]=apply(sampleNumBoot, 2, function(x) mean(x, na.rm = T))  ####average sample sizes across the bootstrap samples
...
write.table(format(sampleNum[7:21,], digits = 0), paste0(DIREC, "Results/sampleSizeSum.txt"), sep="\t", row.names = F, 
            quote = F)
