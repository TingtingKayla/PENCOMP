...
### after finishing running the simulations, we combine the results to obtain empirical bias,
...
####this script produces a file containing correct, misPred, misWeight for each simulation scenario
  for (numSize in 1:length(sampleSizeNum)) {
    ...
    DIREC=paste0("codes/oneTimePointSimulation/sampleSize", sampleSize, "/", outcome, methodFolder) ###directory where simulation results are stored
